...
local function load_shared_lib(so_name)
	...
	tried_paths[#tried_paths + 1] = 'tried above paths but can not load ' .. so_name
...
size_t convert_to_64(char* dest, int num);
]])
...
function _M.base64_bin(s)
	...
	local dlen = base64_decoded_length(slen)
	local dst = get_string_buf(dlen)
	local r_dlen = encoding.b642bin(s, dst)
	if r_dlen == -1 then
		return nil, "invalid input"
	end
	return ffi_string(dst, r_dlen)
...
function _M.base64_long(long)
	local dst = get_string_buf(6)
	local r_dlen = encoding.convert_to_64(dst, long)
	if r_dlen == -1 then
		return nil, "invalid input"
	end
	return ffi_string(dst, r_dlen)
